	...
	if(m_assimpScene)
	{
		...
		for(unsigned int m = 0; m<m_assimpScene->mNumMaterials; m++)
		{
			...
			if(texFound ==  AI_SUCCESS)
				...
				m_textureOfEachMaterial[m].idDiffuse = 0;
			...
			if(texFound ==  AI_SUCCESS)
				...
				m_textureOfEachMaterial[m].idSpecular = 0;
			...
			if(texFound ==  AI_SUCCESS)
				...
				m_textureOfEachMaterial[m].idAmbiant = 0;
			...
			if(texFound ==  AI_SUCCESS)
				...
				m_textureOfEachMaterial[m].idEmissive = 0;
...
void ModelAndTextureLoader_V2::draw()
{
	unsigned int iMesh = 0;
	glBindVertexArray(this->GetMeshVertexArray());
		this->recursiveMeshRendering(this->GetAssimpScene(), this->GetAssimpScene()->mRootNode, &iMesh);
	glBindVertexArray(0);
}

void ModelAndTextureLoader_V2::recursiveMeshRendering(const struct aiScene *sc, const struct aiNode* nd, unsigned int* currentMesh)
{
	...
	for (unsigned int n = 0; n < nd->mNumMeshes; ++n)
	{
		...
		ModelAndTextureLoader_V2::MATERIAL_TEXTUREID textureIDs = this->GetGLtextureOfMaterial(mesh->mMaterialIndex);
		...
		glDrawElementsBaseVertex(GL_TRIANGLES, 
			this->GetNb3PointsFaceAdvMesh(*currentMesh) * 3, GL_UNSIGNED_INT, 
			(void*)(this->GetIndicesMesh(*currentMesh) * sizeof(unsigned int)), 
			this->GetBaseVertexMesh(*currentMesh));
		(*currentMesh)++;
	}
...
void ModelAndTextureLoader_V2::reorderBuffers()
{

}

void ModelAndTextureLoader_V2::FillVBO(
	const struct aiScene *sc, const struct aiNode* nd, unsigned int* currentMesh, unsigned int& VertexOffset, 
