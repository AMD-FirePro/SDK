...
bool initProgram()
{
	...
	if(Validated)
	{
		...
		if(Validated)
		{
			...
			UniformFrustum = glGetUniformLocation(ProgramName, "frustumPlanes");
...
void ExtractFrustum(float frustum[24], glm::mat4 matView, glm::mat4 matProj)
{
   float   proj[16];
   float   modl[16];
   float   clip[16];
   ...
   for (int j=0; j<4; j++)
   {
     proj[4*i+j]=matProj[i][j];  //column-major matrix 
	 modl[4*i+j]=matView[i][j];
   }
   ...
   frustum[4*0+0] = clip[ 3] - clip[ 0];
   frustum[4*0+1] = clip[ 7] - clip[ 4];
   frustum[4*0+2] = clip[11] - clip[ 8];
   frustum[4*0+3] = clip[15] - clip[12];


   /* Extract the numbers for the LEFT plane */
   frustum[4*1+0] = clip[ 3] + clip[ 0];
   frustum[4*1+1] = clip[ 7] + clip[ 4];
   frustum[4*1+2] = clip[11] + clip[ 8];
   frustum[4*1+3] = clip[15] + clip[12];


   /* Extract the BOTTOM plane */
   frustum[4*2+0] = clip[ 3] + clip[ 1];
   frustum[4*2+1] = clip[ 7] + clip[ 5];
   frustum[4*2+2] = clip[11] + clip[ 9];
   frustum[4*2+3] = clip[15] + clip[13];


   /* Extract the TOP plane */
   frustum[4*3+0] = clip[ 3] - clip[ 1];
   frustum[4*3+1] = clip[ 7] - clip[ 5];
   frustum[4*3+2] = clip[11] - clip[ 9];
   frustum[4*3+3] = clip[15] - clip[13];

   /* Extract the FAR plane */
   frustum[4*4+0] = clip[ 3] - clip[ 2];
   frustum[4*4+1] = clip[ 7] - clip[ 6];
   frustum[4*4+2] = clip[11] - clip[10];
   frustum[4*4+3] = clip[15] - clip[14];

   /* Extract the NEAR plane */
   frustum[4*5+0] = clip[ 3] + clip[ 2];
   frustum[4*5+1] = clip[ 7] + clip[ 6];
   frustum[4*5+2] = clip[11] + clip[10];
   frustum[4*5+3] = clip[15] + clip[14];
}
void display()
{
	currentTime = glutGet(GLUT_ELAPSED_TIME); 
	...
	float frustum[24];
	ExtractFrustum(frustum, matView, matProj);
	...
	glUniform4fv(UniformFrustum, 6, frustum);
	if (Window.AddNoise) 
