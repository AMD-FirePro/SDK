  ...
  if(nd->mNumMeshes)
  {
    ...
    for (unsigned int n = 0; n < nd->mNumMeshes; ++n) 
    {
      ...
      for (unsigned int t = 0; t < mesh->mNumFaces; ++t) 
      {
        const struct aiFace* face = &mesh->mFaces[t];
        for(unsigned int i=0; i<face->mNumIndices;i++) 
       // glBufferData(GL_ELEMENT_ARRAY_BUFFER,NbFaces*4*sizeof(unsigned short), Indices, GL_STATIC_DRAW);
          Indices[face->mNumIndices*nbFacesLocal+i] = face->mIndices[i];

        //AverageQuads(mesh->mVertices[Indices[4*nbFacesLocal]], mesh->mVertices[Indices[4*nbFacesLocal+1]],
        //  mesh->mVertices[Indices[4*nbFacesLocal+2]], 
        //  &normals[3*nbFacesLocal]);

        nbFacesLocal++;
      } 

    }
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,NbFaces*3*sizeof(unsigned short), Indices, GL_STATIC_DRAW);

    //glBufferData(GL_ELEMENT_ARRAY_BUFFER,NbFaces*4*sizeof(unsigned short), Indices, GL_STATIC_DRAW); //4 indices per face since we draw quads
